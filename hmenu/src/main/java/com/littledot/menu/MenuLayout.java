...
public class MenuLayout extends ViewGroup {
	...
	public MenuLayout(Context context, AttributeSet attrs) {
		...
		if (attrs != null) {
			...
			a = getContext().obtainStyledAttributes(attrs, R.styleable.MenuLayout, 0, 0);
			mLeftHolderWidth = Math.max(a.getDimensionPixelSize(R.styleable.MenuLayout_leftHolderWidth, 0), 0);
			a.recycle();
	...
	protected int getSuggestedMinimumWidth() {
		return mLeftHolderWidth + (mChildSize + 40) * getChildCount();
	}
	...
	private static Animation createShrinkAnimation(float fromXDelta, float toXDelta, float fromYDelta, float toYDelta,
		...
		final long preDuration = duration /4;
	...
	public void switchState(final boolean showAnimation) {
		if (showAnimation) {
			final int childCount = getChildCount();
			for (int i = 0; i < childCount; i++) {
				bindChildAnimation(getChildAt(i), i, 400);
			}
		}
